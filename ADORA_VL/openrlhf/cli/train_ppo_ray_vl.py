...
def reward_fn(rewards: List[torch.Tensor], labels: List[str], rule_num: int = 0):
    # return torch.stack(rewards).prod(dim=0)
...
def train(args):
    ...
    if args.init_kl_coef == 0:
        print("Due to init_kl_coef = 0, we remove reference model.")
        ref_model = None
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_adora", action="store_true", default=False, help="whether to use Curriculum Learning for GRPO")
    parser.add_argument("--adora_lamda", type=float, default=0.1, help="Lamda in Curriculum Learning")
    ...
    parser.add_argument("--label_key", type=str, default="label", help="JSON dataset key for reference answers")
