...
from openrlhf.models import get_vlm_for_sequence_regression
from openrlhf.trainer import PPOTrainerVL
from openrlhf.trainer.ppo_utils import ExperienceVL
...
class CriticPPOTrainerVL(PPOTrainerVL):
    def ppo_train(self):
    ...
    def training_step(self, experience: ExperienceVL) -> Dict[str, float]:
        return self.training_step_critic(experience)


@ray.remote(num_gpus=1)
class CriticModelRayActorVL(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain, max_steps):
        ...
        self.trainer = CriticPPOTrainerVL(
            strategy,
            actor=None,
            critic=self.critic,
            reward_model=None,
        ...
        with torch.no_grad():
            if pixel_values_intern is not None:
                value = self.critic(
                    ...
                    pixel_values_intern=pixel_values_intern.to(device),
