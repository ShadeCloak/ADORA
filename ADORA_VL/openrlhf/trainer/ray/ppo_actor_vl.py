...
from openrlhf.datasets import PromptDatasetVL, SFTDatasetVL
from openrlhf.models import ActorVL
from openrlhf.trainer import PPOTrainerVL
from openrlhf.trainer.ppo_utils import ExperienceVL, RemoteExperienceMakerVL
from openrlhf.trainer.ray.vllm_engine import batch_vllm_engine_call
from openrlhf.utils import blending_datasets, get_tokenizer_processor_vl
from openrlhf.utils.deepspeed import DeepspeedStrategy
from openrlhf.utils.deepspeed.deepspeed_utils import offload_deepspeed_states, reload_deepspeed_states
from openrlhf.utils.distributed_util import init_process_group

from .launcher_vl import BasePPORole
from .utils import get_physical_gpu_id


class ActorPPOTrainerVL(PPOTrainerVL):
    def __init__(
        self,
        *args,
        ...
        **kwargs,
        ...
        self.experience_maker = RemoteExperienceMakerVL(
            self.actor,
            self.critic,
            self.reward_model,
            self.initial_model,
...
class ActorModelRayActorVL(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        ...
        #         frozen_count += 1
        # strategy.print(f"Froze {frozen_count}/{total_params} parameters based on prefixes: {freeze_prefix}")
        ...
        self.num_update_steps_per_episodes = (
            len(self.prompts_dataset_vl) * args.n_samples_per_prompt // args.train_batch_size * args.max_epochs
        )
    ...
    def prepare_datasets(self):
        ...
        self.prompts_dataset_vl = PromptDatasetVL(
            prompts_data, self.tokenizer, self.processor, args.prompt_max_len, strategy, input_template=args.input_template
        )
        self.prompts_dataloader = strategy.setup_dataloader(
            self.prompts_dataset_vl, args.rollout_batch_size // strategy.world_size, True, True, self.prompts_dataset_vl.collate_fn
        )
        ...
        if args.pretrain_data:
            ...
            pretrain_dataset = SFTDatasetVL(
                pretrain_data.select(
                    range(
                        min(
                            len(pretrain_data), args.max_epochs * len(self.prompts_dataset_vl) * args.n_samples_per_prompt
                        )
                    )
                ),
                self.tokenizer,
