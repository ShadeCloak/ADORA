...
from transformers import AutoModelForCausalLM, BitsAndBytesConfig, AutoModelForVision2Seq, PreTrainedModel, AutoModel
...
class ActorVL(nn.Module):
        ...
        **kwargs,
        ...
        if isinstance(pretrain_or_model, str):
            ...
            if lora_rank > 0:
                ...
                target_modules = find_all_linear_modules(self.model, True)
    ...
    @torch.no_grad()
    def generate(self, input_ids: torch.Tensor, pixel_values: torch.Tensor, image_grid_thw: torch.Tensor, **kwargs) -> Union[
    ...
    def forward(
        ...
        if pixel_values_intern is not None:
            output = self.model(pixel_values_intern, sequences, attention_mask=attention_mask, position_ids=position_ids, image_flags=image_flags)
        else:
