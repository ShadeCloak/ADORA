...
def compute_score_v2(solution_str: str, 
                 ground_truth: Dict[str, str],
                 format_reward: int = 1,
                 answer_reward: float = 1.0) :
    """Computes comprehensive score for model response.
    ...
    if format_correct and answer_text:
        ...
        if pred_status:
            ...
            correct_count = sum(1 for key in gt_status if pred_status.get(key) == gt_status[key])
            total_count = len(gt_status)
            correct_ratio = correct_count / total_count
            answer_score = (2 - (-1.5)) * correct_ratio -1.5
            print(f"  Content validation: {correct_count}/{total_count} correct, Score: {answer_score:.2f}")
            """
            if pred_status == gt_status:
                answer_score = 2
                print("  Content validation: FULL MATCH")
