...
def compute_grpo_outcome_advantage_length_weight_v1(
    token_level_rewards: torch.Tensor,
    eos_mask: torch.Tensor,
    index: torch.Tensor,
    epsilon: float = 1e-6,
    lamda: float = 0.1  # 新增lambda参数控制权重衰减
):
    """
    修改后的GRPO优势计算函数，包含基于响应长度的权重调整
    """
    # 计算每个response的总分和实际长度
    response_length_dim = token_level_rewards.shape[-1]
    non_zero_mask = (token_level_rewards != 0)
    scores = (token_level_rewards * non_zero_mask).sum(dim=-1)
    response_length = eos_mask.sum(dim=1)  # (bs,)
    original_scores = scores.clone()  # 保存原始分数用于条件判断
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            id2original[idx].append(original_scores[i].item())
            id2length[idx].append(response_length[i].item())
        ...
        for idx in id2score:
            ...
            if len(group_scores) == 1:
                ...
                id2mean[idx] = group_scores.mean().item()
                id2std[idx] = group_scores.std().item()
        ...
        for idx in id2original:
            # 获取原始分数和响应长度
            raw_scores = torch.tensor(id2original[idx])
            lengths = torch.tensor(id2length[idx])
            
            # 分割正负样本
            pos_mask = raw_scores > 1.0
            pos_lengths = lengths[pos_mask]
            ...
            # 动态权重逻辑
            if (max_pos > mean_neg) or (pos_lengths.numel() == 0):
                id2weight[idx] = 1.0
            else:
                id2weight[idx] = lamda  # 使用lambda参数

        # 5. 应用权重到标准化后的分数
        for i in range(bsz):
            idx = index[i].item()
            scores[i] *= id2weight[idx]
